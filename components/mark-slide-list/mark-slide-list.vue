            ...
            <view class="slide-box" v-for="(item, index) in listData" :key="index">
                <view class="slide-list" @touchstart="touchStart($event, index)" @touchend="touchEnd($event, index)"
                    ...
                    <view class="now-message-info" hover-class="uni-item--hover" :style="{ width: windowWidth + 'px' }"
                        ...
                        <image class="icon-image" :src="cdnUrl+item.image" v-if="item.goods_status==2">
                        </image>
                        <view class="img" v-else>
                            <image class="icon-image" :src="cdnUrl+item.image"></image>
                            <image class="layer" src="../../static/xiajia.png"></image>
                        </view>
                        <view class="list-right">
                            <view class="list-title" v-if="item.goods_name">{{ item.goods_name }}</view>
                            <view class="list-detail"
                                style="color: #FF3F3F; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">
                                {{item.goods_price!=0?'￥'+item.goods_price/100:''}}
                            </view>
                            <!-- <view class="list-detail"
                                style="color: #FF3F3F; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;margin-top: 5rpx;"
                                v-if="item.goods_type!=0">
                                {{item.goods_type==2&&item.goods_cost!=0?'或':''}}{{item.goods_score_cost!=0?'￥'+item.goods_score_cost/100:''}}{{item.goods_integral&&item.goods_score_cost!=0?'+':''}}{{item.goods_integral!=0?item.goods_integral/100+'积分':''}}
                            </view> -->
                        </view>
                    </view>
