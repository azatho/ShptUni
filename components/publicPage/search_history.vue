		...
		<view v-if="history_list.length!=0">
			...
			<view class="history_list">
				<block v-for="(item,j) in history_list" :key="j">
					<view class="history" v-if="item" @click="jump(item.key_word)">{{ item.key_word }}</view>
				</block>
			</view>
		</view>
...
export default {
	...
	methods: {
		...
		init() {
			let self = this;
			self.request({
				url: 'ShptUapi/public/index.php/Index/search_history',
				data:{
				}
			}).then(res=> {
				if(res.data.success){
		...
		search(){
			if(this.value!=''){
				uni.navigateTo({
					url:'./searchList?value='+this.value+'&type='+this.type
				})
			}else{
				uni.showToast({
					icon:'none',
		...
		empty(){
			let self = this;
			self.request({
				url: 'ShptUapi/public/index.php/Index/delete_history',
				data:{
				}
			}).then(res=> {
				if(res.data.success){
