...
<template>
    <view>
        ...
        <view class="brows" v-else style="padding-bottom: 100rpx;">
            ...
            <view class="record" v-for="(item,i) in list" :key="i" @click="gobuy(item.supplier_id)">
                ...
                <view class="img">
                    <image :src="$imgUrl(item.supplier_icon)" v-if="item.supplier_icon"></image>
                    <view v-else
                ...
                <view class="txt">
                    <view class="tilte">{{item.supplier_name || '暂无名称'}}</view>
    ...
    export default {
        ...
        methods: {
            init(data) {
                let self = this;
                self.request({
                    url: 'ShptUapi/public/index.php/User/collectionOnline',
                    data: data,
                }).then(res => {
                    console.log(res);
