    ...
    <view id="lookBalance">
        <view class="lookTop">
            <view class="lookTopTime">
                <image src="../../../static/1.png" style="border-radius: 10rpx;" mode=""></image>
                ...
                <view class="lookitem">
                    ...
                    <view class="lookitemMonery">
                        <text>{{$returnFloat(consumption)}}</text>
                        <view class="depositAll">
                            <view class="deposit" @click="gopage(0)">提现</view>
                            <view class="deposit1" @click="gopage(1)">充值</view>
                        </view>

                    </view>
                    <view class="depositLog">
                        <text>累计提现：{{$returnFloat(total_extract_consumption)}}</text>
                        <text>提现中：{{$returnFloat(total_in_consumption)}}</text>
                    </view>
                </view>
            </view>
        </view>
    ...
    export default {
        data() {
            return {
                ...
                consumption: 0.00,
                total_in_consumption: 0.00,
                total_extract_consumption: 0.00,
                arrDjiesuan: [],
                arrShouru: [],
                arrZhichu: [],
                arrisShow: [],
        ...
        methods: {
            ...
            async getData(data) {
                ...
                console.log(data)
                self.request({
                    url: 'ShptUapi/public/index.php/user/user_consumption_change',
                    data: data
                }).then(res => {
            ...
            async balance_info() {
                ...
                self.request({
                    ...
                    if (res.data.success) {
                        ...
                        self.total_in_consumption = data.total_in_consumption;
                        self.total_extract_consumption = data.total_extract_consumption;
                    } else {
                        uni.showToast({
                            title: res.data.msg,
                            icon: 'none'
