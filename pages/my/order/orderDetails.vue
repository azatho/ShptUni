	...
	<view>
					...
					<view class="details">
						...
						<view class="xia">
							<view class="univalence">
								{{items.goods_price!=0?'￥'+items.goods_price/100:''}}
								</view>
		...
		<view class="price">
			<view class="goodprice">
				...
				<view class="two">
					{{orderInfo.order_goods_price!=0?'￥'+orderInfo.order_goods_price/100:''}}
				</view>
			</view>
			<view class="goodprice">
			...
			<view class="goodprice">
				...
				<view class="two1">
					{{orderInfo.order_total_price!=0?'￥'+orderInfo.order_total_price/100:''}}
				</view>
			</view>
		</view>
		<!-- 分割线 -->
		<view style="width: 100%; height: 20rpx; background-color: #F5F5F5;"></view>
		<!-- 付款方式 -->
		<view style="background-color: #FFFFFF;height: 70rpx;line-height: 70rpx;padding: 0 30rpx;display: flex;justify-content: space-between;">
			<view style="font-size: 26rpx;">付款方式</view>
			<view style="font-size: 26rpx;">{{orderInfo.order_payment==0?'余额':orderInfo.order_payment==1?'支付宝':orderInfo.order_payment==2?'微信':""}}</view>
		</view>
		<view style="width: 100%; height: 20rpx; background-color: #F5F5F5;"></view>
		<!-- 订单信息 -->
		<view class="order">
		...
		<view class="btn" >
			<view
				:class="orderInfo.order_status==5||orderInfo.order_status==1||orderInfo.order_status==2 ?'btn1':''"
				...
				{{orderInfo.order_status==1?'去支付':orderInfo.order_status==2?'退款':orderInfo.order_status==5?'确认收货':''}}
			</view>
	...
	export default {
		...
		methods: {
			...
			init() {
				let self = this;
				self.request({
					url: 'ShptUapi/public/index.php/Order/orderDetail',
					data: {
			...
			getStatus(e, order_index, order_type, order_total_price, user_coupon) {
				...
				if (e == 6) { //去评价页面
					...
					uni.navigateTo({
						url:"../../shop/payOrder?order_index="+order_index
					})				
				} else if (e == 2) { //退款
					let self = this;
					uni.showModal({
