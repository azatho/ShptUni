	...
	<view>
		...
		<view v-else style="padding-top: 80rpx;">
			<view class="list" v-for="(item,i) in orderList" :key="i">
				<view class="one" >
					<view class="left" style="font-size: 30rpx;font-family: Source Han Sans CN;font-weight: 400;color: #999999;">
						订单号：{{item.order_id}}
					</view>
					...
					<view class="details">
						...
						<view class="xia">
							<view style="display: flex;align-items: center;">
								<view class="univalence">{{items.order_goods_integral/100}}积分</view>
							</view>
				...
				<view class="three">
					<view class="allClass">订单总价：{{item.order_integral/100}}积分</view>
					<view class="btn">
						<!-- 5种按钮状态, 三种按钮样式   goods_status-->
						<view class="btn2" v-if="item.order_status==1" @click="cancleOrder(item.order_index)">取消订单
						</view>
						<view
							:class="item.order_status==9?'btn3':item.order_status==7?'btn3':'btn1'"
							@click="getStatus(item.order_status,item)"
							v-if="item.order_status!=6 && item.order_status!=3&& item.order_status!=2  && item.order_status!=4">
							{{item.order_status==0?'删除订单':item.order_status==1?'去支付':item.order_status==9?'已取消':item.order_status==5?'确认收货':''}}
						</view>
					</view>
				</view>
			</view>
	...
	export default {
		...
		methods: {
			...
			delOrder(e){
				let self = this;
				self.request({
					url:'ShptUapi/public/index.php/Order/del_order',
					data:{
						order_index:e
					}
				}).then(res => {
			...
			init() {
				...
				let self = this
				self.request({
					url: 'ShptUapi/public/index.php/Order/orderList_v2',
					data: {
						page: self.page,
						order_type:5,
					...
					if (res.data.success) {
						uni.hideLoading()
						this.loading = true
						console.log(222,res.data.data.total_page);
			...
			getStatus(e, item) {
				let self = this;
				if (e == 0) { //已取消按钮==>不作操作
					self.delOrder(item.order_index)
					...
					uni.navigateTo({
						url:"../../shop/payCouponOrder?order_index="+item.order_index+'&order_integral='+item.order_integral
					})
