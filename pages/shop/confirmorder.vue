	...
	<view class="">
		...
		<view class="base_msg" v-if='!type'>
			<view class="msg_con">
				...
				<view class="">￥{{goods_info.goods_price/100?goods_info.goods_price/100:'0'}}</view>
			</view>
			<view class="msg_con">
				<view class="">配送费</view>
				<view class="">￥{{goods_info.freight/100||'0'}}</view>
			</view>
			<view class="msg_con">
				<view class="total">订单总价</view>
				<view class="price">{{goods_info.total_price!=0?'￥'+goods_info.total_price/100:'0'}}
				</view>
			</view>
		</view>
	...
	export default {
		...
		methods: {
				...
				self.request({
					...
					if (res.data.data.list.length != 0) {
						for (let i = 0; i < res.data.data.list.length; i++) {
							...
							this.type==1?this.init1():this.init()
						}
					} else {
			...
			init() {
				let self = this
				self.request({
					url: 'ShptUapi/public/index.php/order/confirm_order',
					data: {
			...
			init1(){
				let self = this;
				self.request({
					url:'ShptUapi/public/index.php/Order/OrderInfos',
					data:{
						cart_index:self.cart_index
					}
				}).then(res => {
				if (res.data.success) {
					self.carGoodsInfo = res.data.data;
					self.carGoods_list = res.data.data.info;
					for (let a of res.data.data.info){
						self.carOrder_remark[a.goods_supplier]='';
					}
					} else {
						uni.showToast({
						title: res.data.msg,
			...
			groupAdd() {
				let self = this
				self.request({
					url: 'ShptUapi/public/index.php/Order/addOrder',
					data: {
						goods_index: self.orderInfor.sku_goods,
						sku_index: self.orderInfor.sku_index,
					...
					if (res.data.success) {
						...
						uni.redirectTo({
							url:'payOrder?order_index='+self.order_index
						})
					} else {
						uni.showToast({
							title: res.data.msg,
