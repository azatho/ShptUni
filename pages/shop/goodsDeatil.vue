	...
	<view class="goods">
		...
		<view class="banner">
			...
			<view class="topImg"  v-if="videoBtn==0">
				...
				<view v-if='showVideo'>
					<view  @click="vBtn(0)" :class="videoBtn==0?'videoBtn1':''" class="videoBtn" style="left: 305rpx;" >视频</view>
					<view  @click="vBtn(1)" :class="videoBtn==1?'videoBtn1':''"  class="videoBtn" style="left: 385rpx;">图片</view>
				</view>
		...
		<view class="goodsTitle">
			...
			<view class="price"
				...
				<view class="txt1">{{ '￥'+goods_info.shopInfo.goods_cost / 100}}</view>
		...
		<view class="foot" style='position: fixed;bottom: 0rpx;width: 100%;background-color: #FFFFFF;'>
			<view style="width: 100%;display: flex;align-items: center;justify-content: space-between;">
				...
				<view v-if='goods_info.shopInfo.goods_status==6'
					style="color: #FFFFFF;font-size: 22rpx;height: 100rpx;line-height: 100rpx;text-align: center;width: 440rpx;background: linear-gradient(0deg, #999999, #EEEEEE);">
					距开放购买时间：<text
						v-if="countDownDay">{{countDownDay}}天</text><text>{{countDownHouse<10?'0':''}}{{countDownHouse}}:{{countDownMintue<10?'0':''}}{{countDownMintue}}:{{countDownSecond<10?'0':''}}{{countDownSecond}}</text>
				</view>
				<view v-else style="display: flex;">
					<view class="btn" style="background-color:#454545 ;color: #FFFFFF;" @click="buy(1)">加入购物车</view>
					<view class="btn" style="background: linear-gradient(180deg, #FF6351, #FF672D);color: #FFFFFF;"
						@click="buy(0)">立即购买</view>
				</view>
			</view>
		</view>
		<!-- 客服弹窗 -->
		<u-mask :show="showService" @click="openService" style="text-align: center;">
			<view style="background-color: #FFFFFF;width: 500rpx;height: 600rpx;margin: 0 auto;margin-top: 400rpx;border-radius: 20rpx;">
			...
			<view class="common_con1">
				...
				<view class="spacs_img">
					...
					<view class="">
						<view class="price">
							￥
							<text>{{selectSkuInfo.sku_cost_price/100}}</text>
							<!-- +<text>{{$returnFloat(goods_info.goods_sku_price[0].sku_inventory)}}积分</text> -->
						</view>
						<view>
							<view class="choose">已选：{{ selectSkuInfo.sku_two==''?'无规格':selectSkuInfo.sku_two}}</view>
						...
						<view class="options">
							<block v-for="(items, k) in item.next_sku" :key="k">
								<view :class="items.select ? 'option' : ''"
									@click="getPrice(item.sku_index, items.sku_index, i, k)">{{ items.sku_name }}</view>
							</block>
						</view>
					</view>
				</view>
		...
		<view>
			<uni-popup ref="popup_share" type="bottom">
				...
				<view class="" style="text-align: center;margin-bottom: 80rpx;border-radius: 20rpx;">
					<image :src="$imgUrl(goods_share)" style="z-index: 3;width: 500rpx;height: 800rpx;border-radius: 20rpx;"
						mode=""></image>
				</view>
