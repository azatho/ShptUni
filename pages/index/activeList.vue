            ...
            <view v-for="(item,i) in activeList.list" :key="i" class="concent" @click="goGroupGoods(item)">
                ...
                <view style="display: flex;height: 150rpx;flex-direction:column;justify-content: space-between;">
                    <view style="display: flex;align-items: center;">
                        <view class="font-24" :class="item.status>1?'title1':'title'">{{item.status>1?'拼购':'未开始'}}
                        </view>
                    ...
                    <view style="display: flex;align-items: center;justify-content: space-between;padding-right: 20rpx">
                        <view>
                            ...
                            <view style="color: #999999;font-size: 24rpx;text-decoration: line-through;">
                                ￥{{item.goods_cost/100}}</view>
                        </view>
                        <view :class="item.status==0||item.status==2?'btn':'btn1'" @click.stop="groupBtn(item)">
                            {{item.status==0?'预定轻松拼团':item.status==1?'已预定(点击可关闭)':item.status==2?'去抢购':item.status==3?'轻松拼团中':'参与中'}}
                        </view>
                    </view>
                </view>
            </view>
            ...
            <view
                ...
                <view
                    ...
                    <view
                      ...
                      @click.stop="agreementStatus=1"  @click="sure_shop()">确定</view>
                </view>
            </view>
        </u-mask>
		<!-- 选择抢中结果类型弹窗 -->
        ...
        methods: {
			...
			sure_choose () {
			    ...
			    if (this.agreementStatus == 1) {
			        this.changeGroupStatus(this.iteeem)
			    } else {
            ...
            init() {
                ...
                self.request({
                    ...
                    if (res.data.success) {
                        ...
                        self.activeList.list > 0 ? self.activeList.list = [...self.activeList.list, ...res.data
                            .data.list
                        ] : self.activeList = res.data.data
                    } else {
                        uni.showToast({
                            title: res.data.msg,
                            icon: 'none'
