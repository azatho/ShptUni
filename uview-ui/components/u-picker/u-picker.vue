...
export default {
	...
	mounted() { 
		this.porpMonth = this.startMonth;
		this.propDay = this.startDay;
		this.init();
	},
	computed: {
		propsChange() {
	...
	methods: {
		...
		setMonths() {
			this.months = this.generateArray(this.porpMonth, 12);
			this.valueArr.splice(this.valueArr.length - 1, 1, this.getIndex(this.months, this.month));
		},
		setDays() {
			let totalDays = new Date(this.year, this.month, 0).getDate();
			this.days = this.generateArray(this.propDay, totalDays);
			let index = 0;
			...
			this.day>(this.days.length+this.propDay-1)?this.day = this.days.length:this.day=this.propDay;
