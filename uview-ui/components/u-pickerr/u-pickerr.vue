 ...
 * @property {Boolean} mask-close-able 是否 允许通过点击遮罩关闭Picker（默认true）
...
export default {
	...
	computed: {
		propsChange() {
			// 引用这几个变量，是为了监听其变化
			return `${this.mode}-${this.defaultTime}-${this.startYear}-${this.endYear}-${this.startMouth}-${this.endMouth}-${this.startDay}-${this.defaultRegion}-${this.areaCode}`;
		},
		regionChange() {
			// 引用这几个变量，是为了监听其变化
	...
	methods: {
		...
		setMonths() {
			if (this.isYear===true){
				this.months = this.generateArray(this.startMouth, this.endMouth);
			}
			else{
		...
		change(e) {
			...
			if(this.valueArr[0]=="0"&&this.valueArr[1]=="0"){
				this.isMouth=true
				this.setDays()
			}else{
				this.isMouth=false
